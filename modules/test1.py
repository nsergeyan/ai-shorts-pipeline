...
if LANGUAGE == "ru":
    ...
    MY_NICHES = ["Fallout Universe", "simple interesting facts about Attack on Titan", "simple Ancient History facts",
                 "One-Punch Man", "simple interesting space facts(universe)",
                 "Simple interesting space theories(universe)", "Simple interesting facts about vikings",
...
def generate_idea_from_niche(broad_niche, language="ru"):
    ...
    selected_tier = random.choices(tiers, weights=[0.5, 0.3, 0.2], k=1)[0]
    ...
    - If the topic is "Simple interesting space theories(universe)" -> Make a videos like what will happen if a person goes to black hole, etc. Make it simple for a typical tiktok watcher.
       ...
       Have you ever wondered why Geto betrayed Gojo and turned evil in Jujutsu Kaisen? Let me explain. Well, Geto didn't wake up one day and decide to destroy the world; the world destroyed him first. He used to be the perfect sorcerer: protect the weak, save the helpless, and carry the burden without complaint. That was his whole identity. But everything snapped when Riko died in front of him. What hurt wasn't just Toji killing her; it was the crowd of non-sorcerers celebrating her death like it was entertainment—the same people he swore his life to protect. After that, every mission felt heavier. He kept swallowing curses, absorbing the vomit-tasting negativity humans created, and slowly he began to hate the people he was protecting. Then Yuki told him the truth that broke whatever sanity he had left: non-sorcerers are the source of curses. Sorcerers keep dying because of people who don't even care, and that's when Geto's belief flipped. He didn't want to save non-sorcerers anymore; he wanted a world where sorcerers didn't have to suffer for them. Haibara's death pushed him off the edge completely. On his next mission, he wiped out an entire village of 112 non-sorcerers just to rescue two sorcerer girls, and from that moment, Geto wasn't a protector anymore.
        ...
        4. YOUTUBE QUERY: !!! IMPORTANT !!!
           GOAL: Find high-quality, scenes that focuses on characters and atmosphere. Avoid UI, HUD, and player-controlled movement. Try to get from official sources.
            1. SEARCH STRATEGY:
...
if __name__ == "__main__":
    print("🧪 Running SUBTITLE TEST without any external files...")

    # 1. CREATE A BLANK VIDEO CLIP (black screen, 10 seconds)
    from moviepy.editor import ColorClip

    duration = 10
    test_video = ColorClip(size=(1080, 1920), color=(0, 0, 0), duration=duration)
    test_video_path = "temp_test_video.mp4"
    test_video.write_videofile(
        test_video_path,
        codec="libx264",
        fps=1,  # Low FPS for fast render
        audio=False,
        verbose=False,
        logger=None
    )

    # 2. CREATE SILENT AUDIO USING CompositeAudioClip (more reliable)
    from moviepy.editor import AudioClip, CompositeAudioClip
    import numpy as np

    # Create silent audio clip
    silent_clip = AudioClip(lambda t: np.array([0.0, 0.0]), duration=duration, fps=44100)
    test_audio = CompositeAudioClip([silent_clip])
    test_audio_path = "temp_test_audio.wav"  # Use WAV instead of MP3

    # Write audio file
    test_audio.write_audiofile(
        test_audio_path,
        fps=44100,
        nbytes=2,
        verbose=False,
    ...
    # 3. MOCK SUBTITLE DATA (exact format expected by your function)
    mock_subtitles = [
        (0.0, 2.5, "Hello this is a test"),
        (2.5, 5.0, "Subtitles should appear now"),
        (5.0, 7.5, "If you see this it works"),
        (7.5, 10.0, "Great job fixing it!")
    ]
    ...
    try:
        final_path = merge_audio_video(
            ...
            output_name="subtitle_test_output.mp4",
            vertical=True,
            shorts_cap=False,
            music_path=None,
            music_volume=0.0,
            subtitles_data=mock_subtitles,
        ...
        for f in [test_video_path, test_audio_path]:
            if os.path.exists(f):
                os.remove(f)
